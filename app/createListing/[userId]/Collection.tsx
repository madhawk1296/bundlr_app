...
export default function Collection({ userId, currencies, collection }) {
    ...
    return (
        <Link  href={`/createListing/${userId}/${address}`}>
            <div className='relative w-[250px] rounded-xl overflow-hidden border-2 border-gray-100 cursor-pointer transition-all duration-200 shadow-sm hover:shadow-md'>                    
                ...
                <div className='relative w-full h-fit px-[20px] py-[4px] flex flex-col'>
                    ...
                    <h1 className='relative w-full h-[20px] font-semibold text-sm text-gray-600 tracking-wide overflow-hidden truncate'>{floorPriceValue && formatUSD(floorPriceValue)}</h1>
