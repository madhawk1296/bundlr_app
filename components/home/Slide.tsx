...
export default function Slide({ index, collection }) {
    ...
    return (
        <div className='relative w-[25vw] h-full flex justify-center'>
            <Link href={`/collection/${address}`} className='relative w-[85%] aspect-[1/1] rounded-xl overflow-hidden border-2 border-gray-100 cursor-pointer hover:shadow-md'> 
                <div className='absolute top-0 left-0 w-full h-full'>
                    <Image alt={`Slide ${index}`} src={imageUrl} fill={true} sizes={`${25 * .85}vw`} priority={true} />     
                </div> 
                ...
                <div className='absolute bottom-0 w-full h-[20%] px-[20px] pt-[4px] flex flex-col'>
                    <h1 className='relative w-full h-[28px] font-semibold text-lg text-white tracking-wide overflow-hidden truncate'>{name}</h1>
                    <div className="relative h-[28px] flex flex-inline gap-2 items-center">
