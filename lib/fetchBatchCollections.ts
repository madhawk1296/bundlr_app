export default async function fetchBatchCollections(batch) {
    ...
    return (await (await fetch(`https://api.simplehash.com/api/v0/nfts/collections/${process.env.CHAIN_ID}?limit=50&contract_addresses=${batchFormatted}`, options)).json()).collections;
}
