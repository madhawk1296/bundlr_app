export default async function fetchCollectionItemsByUser(user, collection) {
    const options: any = {
      ...
      return (await (await fetch(`https://api.simplehash.com/api/v0/nfts/owners?chains=arbitrum&wallet_addresses=${user}&contract_addresses=${collection}&limit=50`, options)).json()).nfts;
    }